classdef MechFluidFixedStressSplitModelTEDFM < MechFluidSplitModelTEDFM
%
%
% SYNOPSIS:
...
%}
    ...
    methods
        function model = MechFluidFixedStressSplitModelTEDFM(G, rock, fluid, ...
                                                        mech_problem, varargin)

            model = model@MechFluidSplitModelTEDFM(G, rock, fluid, mech_problem, ...
                                         varargin{:});
        end
        ...
        function fluidModel = setupFluidModel(model, rock, fluid, fluidModelType, ...
            ...
            switch fluidModelType
              case 'water'
                fluidModel = WaterFixedStressFluidModelTEDFM(model.G, rock, fluid, varargin{:});
                case 'oil water'
                fluidModel = OilWaterFixedStressFluidModelTEDFM(model.G, rock, fluid, varargin{:});
                case 'water gas'
                fluidModel = WaterGasFixedStressFluidModelTEDFM(model.G, rock, fluid, varargin{:}); %modified WaterGasFixedStressFluidModel to WaterGasFixedStressFluidModelTEDFM
              case 'blackoil'
                fluidModel = BlackOilFixedStressFluidModelTEDFM(model.G, rock, fluid, varargin{:});
              otherwise
        ...
        % preserved.
            ...
            if griddim == 3
                pI = bsxfun(@times, p(1:model.G.Matrix.cells.num), [1, 1, 1, 0, 0, 0]);
                nlin = 6;
            else
                pI = bsxfun(@times, p(1:model.G.Matrix.cells.num), [1, 1, 0]);
                nlin = 3;
            end
