classdef GasWaterFluidModel < TwoPhaseWaterGasModel
%
%
% SYNOPSIS:
...
%}
    ...
    methods
        function model = GasWaterFluidModel(G, rock, fluid, varargin)
        % Constructor
        ...
        function [problem, state] = getEquations(model, state0, state, dt, ...
                                                        drivingForces, ...
                                                        varargin)
            error(['The GasWaterFluidModel is not meant for being called directly. ' ...
        ...
        % list all the variables that are recognized and can be handled by the model
            fds = {'wellSol', 'pressure', 's', 'sW',  'sG', 'water', 'gas'};
        end
        ...
        function [state, report] = updateState(model, state, problem, dx, ...
            ...
            [state, report] = updateState@TwoPhaseWaterGasModel(model, state, ...
                                                              problem, dx, ...
                                                              drivingForces);
