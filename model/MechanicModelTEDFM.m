classdef MechanicModelTEDFM < PhysicalModel
%
...
%}
    ...
    methods
        function model = MechanicModelTEDFM(G, rock, mech_problem, varargin)
...
%             model.G = G.Matrix;
            ...
            % Process the grid for mechanical computation
            if any(strcmpi('createAugmentedGrid', model.G.Matrix.type))  % modified model.G.type to model.G.Matrix.type
                model.G = createAugmentedGrid(model.G.Matrix); %modified model.G to model.G.Matrix
            end


            % Physical properties of rock and fluid
            model.mech  = mech_problem;
            model.rock  = G.Matrix.rock; %modified the rock to G.Matrix.rock
%             model.rock  = rock; 

            % Compute stiffness tensor C, if not given
%             if ~isfield(model.mech, 'C')
%                 [model.mech.C, model.mech.invC, model.mech.invCi] = ...
%                     Enu2C(model.mech.E, model.mech.nu, model.G); %modified model.G to model.G.Matrix
%             end

            % Compute stiffness tensor C, if not given
            if ~isfield(model.mech, 'C')
                [model.mech.C, model.mech.invC, model.mech.invCi] = ...
                    Enu2C(model.mech.E, model.mech.nu, model.G.Matrix); %modified model.G to model.G.Matrix
            end
            ...
            if isempty(opt.InputModel)
                ...
                operators = setupOperatorsVEMTEDFM(model.G, ...
                                              model.mech.C, ...
                                              model.mech.el_bc, ...
                                              model.mech.load, ...
                                              alpha_scaling, S);
...
%                               alpha_scaling, S);
        ...
        function [problem, state] = getEquations(model, state0, state, dt, ...
            ...
            eqs = equationsPoroMechanicsTEDFM(xd, model, fluidp);
        ...
        function [state, report] = updateState(model, state, problem, dx, drivingForces)
            ...
            state = addDerivedQuantitiesTEDFM(model, state);
        end
