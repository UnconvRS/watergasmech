...
model = MechFluidFixedStressSplitModel(G, rock, fluid, mech, ...
                                       'fluidModelType', 'water gas', ...
                                       'splittingTolerance', opt.splittingTolerance, ...
...
zticklabels([]);
cb = colorbar;                 
cb.Position(3) = cb.Position(3) * 1.1;  
...
% time_years = cumsum(schedule.step.val)/year;
% 
% sat_endInj = states{100}.s(:,2);  % co2 saturation at end of injection 100 years
% sat_mid = states{150}.s(:,2);  % co2 saturation at end state 650 years
sat_end = states{end}.s(:,2);  % co2 saturation at end state 1300 years

% plume_cells1 = sat_endInj > 0.0001;
% plume_cells2 = sat_mid > 0.0001;
plume_cells3 = sat_end > 0.0001;


% figure,
% plotGrid(G, 'facecolor', 'none','edgeAlpha',0.1);  % plot outline of simulation grid
% plotGrid(G, plume_cells1, 'facecolor', 'red'); % plot cells with CO2 in red
% plotGrid(G, G.nnc.cells(:,2), 'faceColor', 'y','faceAlpha',0.1, 'edgeColor', 'k');
...
% figure,
% plotGrid(G, 'facecolor', 'none','edgeAlpha',0.1);  % plot outline of simulation grid
% plotGrid(G, plume_cells2, 'facecolor', 'red'); % plot cells with CO2 in red
% plotGrid(G, G.nnc.cells(:,2), 'faceColor', 'y','faceAlpha',0.1, 'edgeColor', 'k');
% plotWellData(G, W,  'TextColor', 'blue', 'color', [0 0 1]);
% axis off
% view(-75,30)
...
% 
% figure;
% plot(cumsum(schedule.step.val)/day,injres);
% xlabel('Time, years');
% ylabel('Amount Injected, r-m3')
% title('Rate at reservoir condition')
...
% injectedCO2res=[];
% for i=1:numel(states)
%     curInjectionres=wellSols{i}.qGr;  
%     injectedCO2res=[injectedCO2res;curInjectionres];
% end
% 
% cumeCO2injectedres=cumtrapz(cumsum(schedule.step.val),injectedCO2res);
% 
% figure;
% plot(cumsum(schedule.step.val)/day,cumeCO2injectedres);
% xlabel('Time, years');
% ylabel('Amount Injected, r-m3')
% title('Volume at reservoir condition')
...
c_global = false(G.cartDims);
c_global(1, 1, 1) = true;
cindex = find(c_global(G.cells.indexMap));

cindexstress = initState.stress(cindex, 1:3);
cindexstrain = initState.strain(cindex, 1);

entirestressend = states{end}.stress(:, :);

cindexstressend = entirestressend(cindex, :);

cindexpressure = initState.pressure(cindex,1);

cindexdisplacement = initState.uu(cindex,1:3);

stresselbc = el_bc.force_bc.force(:, :);

cindexstressbc = stresselbc(cindex, :);

connectedCells = G.faces.neighbors(6777, :);


%Change the stress and strain sign convention to match mrst by multiplying
%by -1

statesmodified = states;

for i=1:numel(states)
    statesmodified{i}.stress = statesmodified{i}.stress(:, :) .* -1;
    statesmodified{i}.strain = statesmodified{i}.strain(:, :) .* -1;
end

figure;
plotToolbar(G, statesmodified, 'outline', true); view(-4, 32); axis tight;
% view(-63, 50);
colormap jet;
set(gcf, 'position', [152   419   689   404]);
set(gca, 'XTick', [], 'YTick', [], 'ZTick', []);
...
layer4_global = false(G.cartDims);
layer4_global(1:end, 1:end, 4) = true;
layer4_index = find(layer4_global(G.cells.indexMap));

figure,
plotGrid(G, 'facecolor', 'none','edgeAlpha',0.1);  
plotGrid(G, layer4_index, 'facecolor', 'y'); 

rockpermlayer4 = rock.perm(layer4_index, :);


layer1_global = false(G.cartDims);
layer1_global(1:end, 1:2, 1) = true;
layer1_index = find(layer1_global(G.cells.indexMap));

stressAll = statesmodified{49, 1}.stress(:, :);
stressAll = stressAll./kilo;

strainAll = statesmodified{49, 1}.strain(:, :);
