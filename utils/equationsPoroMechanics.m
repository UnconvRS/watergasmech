...
%
% SEE ALSO:
%   MechBlackOilModel, MechOilWaterModel, MechWaterModel, MechWaterGasModel
...
%}
    ...
    G = model.G; % modified from model.G 
    s = model.operators;
    alpha = model.rock.alpha; %modified it from model.rock.alpha

    eqs{1} = s.A * x - s.gradP * (alpha .* fluidp) - s.rhs;

    % normalization constant
    fac =  1 / (1e6 * mean(G.cells.volumes)); % modified from G.cells.volumes
    eqs{1} = eqs{1} * fac;
    names = {'disp'};
    types = {'disp_dofs'};
