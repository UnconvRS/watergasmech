function [eqs, names, types] = equationsPoroMechanicsTEDFM(x, model, fluidp)
%
%
% SYNOPSIS:
...
%}
    ...
    G = model.G;
%     G = model.G.Matrix; % modified from model.G to model.G.Matrix
    s = model.operators;
    alpha = model.G.Matrix.rock.alpha; %modified it from model.rock.alpha to model.G.rock.alpha

    eqs{1} = s.A * x - s.gradP * (alpha .* fluidp(1:G.Matrix.cells.num)) - s.rhs;

    % normalization constant
    fac =  1 / (1e6 * mean(G.Matrix.cells.volumes)); % modified from G.cells.volumes to G.Matrix.cells.volumes
    eqs{1} = eqs{1} * fac;
    names = {'disp'};
    types = {'disp_dofs'};
