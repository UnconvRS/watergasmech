function [eqs, names, types, state] = equationsWaterGasMechTEDFM(p0, sW0, state0, ...
                                                            p, sW, wellVars, ...
                                                            state, model, ...
                                                            dt, mechTerm, ...
...
% Equations for water-gas flow with mechanical effects on porosity.
    ...
    effPorVol  = rock.poro .* (G.Matrix.cells.volumes .* pvMult(1:G.Matrix.cells.num))  + rock.alpha .* mechTerm.new;
    effPorVol0 = rock.poro .* (G.Matrix.cells.volumes .* pvMult0(1:G.Matrix.cells.num)) + rock.alpha .* mechTerm.old;
    % ----------------------------------------------------------------------------
